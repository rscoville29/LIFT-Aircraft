...
export function setCalendarToDate(date) {
  ...
  let bookings = getBookings(dateFrom, dateTo)
    .then(function (result) {
      let rows = [];
      $w("#searchingText").text =
        result._items.length == 0
          ? "No bookings found for that day!"
          : "Select booking to check in:";
      $w("#searchingText").show;

      result._items.forEach((element) => {
        let ssn = getSessionOfBooking(element._id).then(function (session) {
          let time = new Date(session.start.timestamp).toLocaleTimeString(
            "en-US",
            { hour: "2-digit", minute: "2-digit" }
          );
          let firstName = element.formInfo.contactDetails.firstName;
          let lastName = element.formInfo.contactDetails.lastName;
          if (element.totalParticipants != 1)
            lastName = lastName.concat(
              " (+" + (element.totalParticipants - 1).toString() + ")"
            );
          rows.push({
            id: element._id,
            title:
              ...
              lastName +
              "</p6><br>" +
              "<p6 style ='font-size:small'>" +
              element.bookedEntity.title +
              "</p6><br>" +
              "<p6 style ='font-size:small'>" +
              time +
              " @ " +
              element.bookedEntity.location.businessLocation.name +
              "</p6><br>",
            name: firstName + " " + lastName,
            datetime: new Date(session.start.timestamp).toLocaleDateString(
              "en-US",
              {
                year: "numeric",
                month: "short",
...
export function dayTable_rowSelect(event) {
  ...
  $w("#selectedTimeAndLocation").text =
    event.rowData.datetime + " @ " + event.rowData.location;

  $w("#selectedSessionSection").expand();
  $w("#selectedSessionSection").scrollTo();

  getBooking(event.rowData.id).then((result) => {
    booking = result;
...
async function refreshBookingInputTable(book) {
    ...
    if (partySize > i - 1) {
      ...
      if (emails !== undefined && emails.length > i - 1) {
        getContactByEmail(emails[i - 1]).then((result) => {
          ...
          $w("#gender" + i.toString()).value =
            result.info.extendedFields["custom.gender"] === undefined
              ? "undisclosed"
              : result.info.extendedFields["custom.gender"];
          $w("#waiver" + i.toString()).checked =
            waivers == undefined ? false : waivers[i - 1];
          $w("#email" + i.toString()).value = result.primaryInfo.email;
          $w("#weight" + i.toString()).value =
            result.info.extendedFields["custom.lastknownwt"] === undefined
              ? ""
              : result.info.extendedFields["custom.lastknownwt"];
        });
      }
    }
  }
  ...
  getContact(book.formInfo.contactDetails.contactId).then((result) => {
    contact = result;
    $w("#firstName1").value = contact.info.name.first;
    $w("#lastName1").value = contact.info.name.last;
    $w("#gender1").value =
      contact.info.extendedFields["custom.gender"] === undefined
        ? "undisclosed"
        : contact.info.extendedFields["custom.gender"];
    $w("#email1").value = contact.primaryInfo.email;
    $w("#waiver1").checked = waivers == undefined ? false : waivers[0];
    $w("#weight1").value =
      contact.info.extendedFields["custom.lastknownwt"] === undefined
        ? ""
        : contact.info.extendedFields["custom.lastknownwt"];
  });
}

export async function getCurrentMember(options) {
  return await wixMembers.currentMember
    .getMember(options)
    .then((member) => {
